		...
		private static void LoadTrains()
		{
			...
			for (int k = 0; k < TrainManager.Trains.Length; k++)
			{
				...
				if (TrainManager.Trains[k].State == TrainManager.TrainState.Bogus)
				{
					...
					TrainDatParser.ParseTrainData(OpenBveApi.Path.CombineFile(Folder, "train.dat"), System.Text.Encoding.UTF8, TrainManager.Trains[k]);
					System.Threading.Thread.Sleep(1); if (Cancel) return;
				...
				else
				{
					...
					TrainDatParser.ParseTrainData(OpenBveApi.Path.CombineFile(CurrentTrainFolder, "train.dat"), CurrentTrainEncoding, TrainManager.Trains[k]);
					TrainProgressCurrentSum += TrainProgressCurrentWeight;
					System.Threading.Thread.Sleep(1); if (Cancel) return;
			...
			for (int i = 0; i < TrainManager.Trains.Length; i++)
			{
				if (TrainManager.Trains[i].State != TrainManager.TrainState.Bogus)
				{
					...
					{
						//All non-player trains currently use the OpenBVE ATS plugin
						//TODO: Allow .Net plugins which state they support AI to work on AI trains
						PluginManager.LoadDefaultPlugin(TrainManager.Trains[i], CurrentTrainFolder);
					}
				}
			}
